...
- **Thumb reach.** With integer pillarboxing, a 19.5:9 phone renders the game at
  4× (1600×960) inside a 2556×1179 screen, leaving ~478 px black bars each side.
  The notch and home indicator fall harmlessly in those bars — but so do the
  physical screen corners, which means the on-screen buttons sit well inboard of
  where a thumb naturally rests. Play it before deciding whether to fix; the
  options are a non-integer scale mode, or drawing the touch overlay in real
  screen space instead of viewport space. Do not change this blind.
