...
#### Full-screen fit and touch placement — solved

The game renders one **400x240 world screen** into a SubViewport, centred in a
root viewport that expands to the device's aspect. The screen-flip camera still
sees exactly one screen, so level composition is untouched, and the leftover
margin holds the touch controls:

| Device | Root viewport | Margin | Buttons |
|---|---|---|---|
| 19.5:9 phone | 522x240 | 61 px each side | right margin, stacked |
| 4:3 iPad | 400x300 | 30 px top/bottom | bottom margin |
| exact 5:3 | 400x240 | none | overlay fallback |

This replaced the earlier pillarbox, where 311 px of black sat each side and the
Jump button sat about a centimetre inboard of the screen edge. The buttons are
now outside the play area entirely.

`tools/shot.sh --res=WxH` captures at a given device aspect;
`tools/genstoreshots.sh` uses it to shoot the phone and tablet layouts
separately, so the store page shows what a player actually sees.

#### One thing to watch on a real device
...
- **Thumb reach** is addressed by the layout above, but has still never been
  felt by a human on hardware. The buttons are where they should be
  geometrically; whether they are the right *size* is a playtest question.
